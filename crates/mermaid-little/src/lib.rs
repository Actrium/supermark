...
// This crate is a large, byte-parity-oriented port of Mermaid's JS renderer.
// Keep the clippy baseline explicit so workspace-level `-D warnings` can run
// without forcing broad behaviour-risky rewrites of ported layout code.
#![allow(
