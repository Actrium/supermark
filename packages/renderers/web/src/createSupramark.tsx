...
/**
 * v0.2 Supramark factory — consumed by files generated by `@supramark/cli`.
 *
 * Takes a `{ engines, features }` spec, captures it in a closure, and returns a
 * `Supramark` React component. Business code only ever sees `<Supramark markdown={...} />`;
 * engines / features stay entirely hidden.
 *
 * @example (generated files write this usage automatically)
 * ```ts
...
/** Props exposed by the FC returned by createSupramark. */
export interface SupramarkFCProps
  extends Omit<SupramarkWebProps, 'config' | 'containerRenderers'> {
  // The spec is already baked into the factory closure, so engines/features are not accepted here.
}

/**
 * Generate a pure markdown → HTML string function from a spec.
 *
 * Internally uses react-dom/server.renderToString; engines / features are inherited from the spec.
 */
...
export function createSupramark(spec: RenderSpec): React.FC<SupramarkFCProps> {
  ...
  // Convert spec.features into the internal SupramarkConfig (used by the parser).
  // Simplified handling: features look like { gfm: true, math: { engine: 'mathjax' } },
  // converted into `config.features = [{ id: '@supramark/feature-gfm', enabled: true, options }, ...]`.
...
// ----------------------------------------------------------------------------
// Simplified features mapping: {gfm:true} → the features array of SupramarkConfig
// ----------------------------------------------------------------------------
