...
function extractSvg(adaptor: MathJaxRenderer['adaptor'], node: unknown): string {
  ...
  const svgNode =
    asAny && typeof adaptor.firstChild === 'function'
      ? adaptor.firstChild(asAny)
      : null;
...
function normalizeMathSvg(svg: string): string {
  ...
  normalized = normalized.replace(
    ...
    (_match, rawAttrs, inner) => {
      ...
      const hasViewBox =
        viewBoxParts.length === 4 && viewBoxParts.every(value => Number.isFinite(value));

      let transformParts: string[] = [];
      if (Number.isFinite(x) || Number.isFinite(y)) {
        transformParts.push(`translate(${Number.isFinite(x) ? x : 0}, ${Number.isFinite(y) ? y : 0})`);
      }
