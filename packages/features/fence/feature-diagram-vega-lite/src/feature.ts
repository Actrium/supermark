...
export const diagramVegaLiteFeature: SupramarkFeature<SupramarkDiagramNode> = {
  metadata: {
    ...
    description:
      'Support for Vega-Lite based diagrams rendered via the unified diagram pipeline.',
    license: 'Apache-2.0',
      ...
      examples: [
        {
          ...
          engine: 'vega-lite',
          code:
            '{ "mark": "bar", "encoding": { "x": { "field": "category" }, "y": { "field": "value" } }, "data": { "values": [{ "category": "A", "value": 1 }] } }',
  ...
  renderers: {
    ...
    web: {
      ...
      infrastructure: {
        ...
        clientScriptBuilder: () =>
          '<!-- Vega-Lite scripts provided by @supramark/web-diagram -->',
      },
      dependencies: [
        {
          name: 'vega',
      ...
      cases: [
        {
          ...
          validate: (result: unknown) => {
            ...
            return children.some(
              (n: any) => n.type === 'diagram' && typeof n.engine === 'string',
            );
          },
          platforms: ['web', 'rn'],
        },
      ...
      functions: [
        {
          ...
          description: '创建带有强类型 options 的 FeatureConfig，方便在 SupramarkConfig.features 中使用。',
          parameters: [
            { name: 'enabled', type: 'boolean', description: '是否启用该 Feature', optional: false },
            { name: 'options', type: 'DiagramVegaLiteFeatureOptions', description: '可选配置项', optional: true },
          ],
