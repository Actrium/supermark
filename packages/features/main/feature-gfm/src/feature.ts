  ...
  syntax: {
    ast: {
      ...
      selector: (node: SupramarkNode) => {
        ...
        if (node.type === 'list_item' && ('checked' in node && node.checked !== undefined)) return true;
        if (
          node.type === 'table' ||
          node.type === 'table_row' ||
      ...
      cases: [
        {
          ...
          validate: (result) => {
            ...
            const hasDelete = nodes.some((n: any) =>
              n.type === 'paragraph' &&
              n.children?.some((c: any) => c.type === 'delete')
            );
            const hasTaskList = nodes.some((n: any) =>
              n.type === 'list' &&
              n.children?.some((item: any) => item.checked !== undefined)
            );
        ...
        },
        {
          ...
          fields: [
            ...
            },
            {
              name: 'checked',
              type: 'boolean | null',
              description: '任务完成状态：true（已完成）、false（未完成）、null/undefined（普通列表项）',
              required: false,
            },
            {
              name: 'children',
      ...
      functions: [
        {
          ...
          description: '创建 GFM Feature 配置对象，用于在 SupramarkConfig 中启用 GitHub Flavored Markdown 扩展',
          parameters: [
            {
              name: 'enabled',
              type: 'boolean',
      ...
      types: [
        {
          ...
          description: 'GFM Feature 配置类型，是 FeatureConfigWithOptions<GFMFeatureOptions> 的类型别名',
...
export function getGFMFeatureOptions(
  config?: SupramarkConfig
): GFMFeatureOptions | undefined {
  return getFeatureOptionsAs<GFMFeatureOptions>(
    config,
