        ...
        },
        {
          ...
          fields: [
            ...
            },
            {
              name: 'value',
              type: 'string',
              description: '文本内容，其中的 Emoji 已经由 markdown-it-emoji 从短代码（:smile:）转换为 Unicode 字符（😄）',
              required: true,
            },
          ],
        },
      ...
      functions: [
        {
          ...
          description: '创建 Emoji Feature 配置对象，用于在 SupramarkConfig 中启用 Emoji 短代码支持',
          parameters: [
            {
              name: 'enabled',
              type: 'boolean',
      ...
      types: [
        {
          ...
          description: 'Emoji Feature 配置类型，是 FeatureConfigWithOptions<EmojiFeatureOptions> 的类型别名',
    ...
    faq: [
      {
        ...
        answer: '支持 GitHub 风格的 emoji 短代码，完整列表可参考 GitHub Emoji API 或 markdown-it-emoji 文档。',
      },
      {
        question: 'Emoji 在 AST 中如何表示？',
        answer: 'Emoji Feature 不创建单独的 AST 节点类型，而是将短代码转换为 Unicode 字符后嵌入到 text 节点的 value 中。',
      },
      {
...
export function getEmojiFeatureOptions(
  config?: SupramarkConfig
): EmojiFeatureOptions | undefined {
  return getFeatureOptionsAs<EmojiFeatureOptions>(
    config,
