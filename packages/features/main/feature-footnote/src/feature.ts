      ...
      cases: [
        {
          ...
          validate: (result) => {
            ...
            const hasReference = nodes.some((n: any) =>
              n.type === 'paragraph' &&
          ...
          validate: (result) => {
            ...
            const references = nodes.reduce((count: number, n: any) => {
              if (n.type === 'paragraph' && Array.isArray(n.children)) {
                return count + n.children.filter((c: any) => c.type === 'footnote_reference').length;
              }
              return count;
            }, 0);
...
const options = getFootnoteFeatureOptions(config);`,
      ...
      types: [
        {
          ...
          description: 'Footnote Feature 配置类型，是 FeatureConfigWithOptions<FootnoteFeatureOptions> 的类型别名',
          definition: 'type FootnoteFeatureConfig = FeatureConfigWithOptions<FootnoteFeatureOptions>',
        },
      ],
    },
    ...
    faq: [
      ...
      },
      {
        ...
        answer: '脚注定义可以包含段落、列表、代码块、引用块等多种 Markdown 元素，支持复杂的内容结构。',
      },
    ],
  },
};
...
export type FootnoteFeatureConfig =
  FeatureConfigWithOptions<FootnoteFeatureOptions>;
...
export function getFootnoteFeatureOptions(
  ...
  return getFeatureOptionsAs<FootnoteFeatureOptions>(
    config,
