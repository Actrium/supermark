...
export const mathFeature: SupramarkFeature<
  SupramarkMathInlineNode | SupramarkMathBlockNode
> = {
  metadata: {
      ...
      interface: {
        ...
        fields: {
          type: {
            type: 'string',
            description:
              '节点类型，行内公式为 "math_inline"，块级公式为 "math_block"。',
          },
          value: {
            type: 'string',
      ...
      cases: [
        {
          ...
          validate: (result) => {
            ...
            const hasMathInline = nodes.some((n: any) =>
              n.type === 'paragraph' &&
              n.children?.some((c: any) => c.type === 'math_inline')
            );
      ...
      types: [
        {
          ...
          description: 'Math Feature 配置类型，是 FeatureConfigWithOptions<MathFeatureOptions> 的类型别名',
    ...
    faq: [
      {
        ...
        answer: '支持标准 LaTeX 数学模式的大部分语法，具体取决于所选的渲染引擎（KaTeX 或 MathJax）。',
      },
      {
        question: '如何切换渲染引擎？',
...
export function getMathFeatureOptions(
  config?: SupramarkConfig
): MathFeatureOptions | undefined {
  return getFeatureOptionsAs<MathFeatureOptions>(
    config,
