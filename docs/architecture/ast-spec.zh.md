...
- `fence_closed` 表示源码中是否存在真实 closing fence；`false` 表示 parser 按 CommonMark 在 EOF 自动闭合。反序列化缺失该字段时按 `false` 处理（向后兼容早于该字段的持久化 AST），消费方据此保守推迟渲染。
...
- `semantic` 是可选的语义 AST 信封，形状为 `{ "engine": "...", "kind": "...", "data": {...} }`：`engine` 为引擎 id，`kind` 标识图形语义类别，`data` 为该类别的结构化语义数据。默认懒解析——parser 主流程不填充，缺省为 `None` 且不序列化（无该字段即等同未解析）；由下游按需解析后填充。详见 `docs/architecture/diagram-semantic-ast.zh.md`。
