...
import { Supramark, DiagramEngineProvider } from '@supramark/web/client';
import {
  featureRegistry,
  findFeature,
  loadFeature,
...
export function FeaturePreview({ initialFeature }: { initialFeature: string }) {
  ...
  const [containerRenderers, setContainerRenderers] = useState<Record<string, any>>({});
  ...
  const loadAndSwitchFeature = useCallback(async (entry: FeatureEntry) => {
    setIsLoading(true);
    const loaded = await loadFeature(entry.shortName);
    setFeature(loaded);
    if (loaded.containerRenderers) {
      setContainerRenderers(prev => ({ ...prev, ...loaded.containerRenderers }));
    }
    setSelectedExample(0);
    setMarkdown(loaded.examples[0]?.markdown ?? '');
    setDirty(false);
    setIsLoading(false);
  }, []);

  useEffect(() => {
    void loadAndSwitchFeature(initial);
  }, [initial, loadAndSwitchFeature]);

  const switchFeature = (shortName: string) => {
    const entry = findFeature(shortName);
    if (!entry) return;
    void loadAndSwitchFeature(entry);
      ...
      >
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <select
                value={feature?.shortName ?? initial.shortName}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          >
            ...
            >
              {isLoading ? (
                <div style={{ color: bgMode === 'dark' ? '#ddd' : '#666', fontSize: 13 }}>
                  Loading feature...
                </div>
              ) : (
                <Supramark
